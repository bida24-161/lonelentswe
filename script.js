...
// Make updateHeaderCartCount globally accessible for any page that loads this script
window.updateHeaderCartCount = () => {
    const cartCountSpan = document.querySelector('.cart-count');
    const currentCart = JSON.parse(localStorage.getItem('cart')) || [];
    if (cartCountSpan) { // Check if the element exists on the current page
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const close = document.getElementById('close'); // Ensure you have <i id="close" class="fa-solid fa-times"></i> in your HTML
    ...
    addToCartButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            const productImageUrl = button.dataset.productImageUrl; // Get the image URL
            ...
            if (existingItem) {
                ...
                cart.push({
                    ...
                    price: productPrice,
                    quantity: 1,
                    imageUrl: productImageUrl // Store the image URL
                });
            }
    ...
    // Only set up modal listeners if the modal elements exist on the page
    if (modalContainer && modalTitle && modalBody && closeButton) {
        window.openModal = (title, content) => { // Make openModal global
        ...
        const aboutUsLink = document.getElementById('aboutUsLink'); // Assuming you added IDs in HTML
        const signInLink = document.getElementById('signInLink');
        const myWaitlistLink = document.getElementById('myWaitlistLink');
        const trackMyOrderLink = document.getElementById('trackMyOrderLink');
        ...
        // Add more modal content for other footer links if needed
        if (aboutUsLink) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveCart = () => {
        ...
        // Call global header cart count update if it exists
    ...
    const calculateCartTotals = () => {
        ...
        // For simplicity, shipping is free. Adjust as needed.
    ...
    const updateQuantity = (productId, newQuantity) => {
        ...
        if (item) {
            ...
            if (isNaN(quantity) || quantity < 1) { // Ensure quantity is a positive number
                 removeItem(productId); // Remove if quantity is invalid or zero/negative
            } else {
